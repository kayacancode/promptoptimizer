...
export function createSubmitPromptTool(client: BestMateClient): Tool {
  return {
    ...
    description: 'Submit a prompt to BestMate for optimization analysis',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
        ...
        model: {
          type: 'string',
          description: 'Model to use for optimization (claude-4-opus, claude-3-sonnet, claude-3-haiku, gpt-4, gemini-pro)',
          enum: ['claude-4-opus', 'claude-3-sonnet', 'claude-3-haiku', 'gpt-4', 'gpt-3.5-turbo', 'gemini-pro'],
          default: 'claude-4-opus'
        },
        temperature: {
          type: 'number',
          description: 'Temperature for model creativity (0.0-1.0)',
          minimum: 0.0,
          maximum: 1.0,
          default: 0.3
        },
