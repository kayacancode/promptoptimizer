...
export function PromptOptimizationFlow({ onOptimizationComplete }: PromptOptimizationFlowProps = {}) {
  ...
  const optimizePrompt = async () => {
    ...
    try {
      ...
      if (data.data.autoOptimization) {
        const autoResult = data.data.autoOptimization
        const originalScore = data.data.overallImprovement || 0
        const improvedScore = autoResult.selectedCandidate.score || 0
        
        if (autoResult.status === 'success' && improvedScore > originalScore) {
          // Show auto-optimization notification
          alert(`🤖 Auto-improved from ${Math.round(originalScore)}% → ${Math.round(improvedScore)}%!\n\nStrategy: ${autoResult.strategy}\nReason: ${autoResult.trigger.reason}`)
        }
      }
