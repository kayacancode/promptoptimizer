...
export class BenchmarkRunner {
  ...
  private async generateModelResponse(prompt: string, modelName: string): Promise<string> {
    try {
      switch (modelName) {
        ...
        case 'claude-3-5-sonnet-20240620':
          if (!process.env.ANTHROPIC_API_KEY) {
            return `Error: Anthropic API key not configured for ${modelName}`
          }
        ...
        case 'gpt-4':
          if (!process.env.OPENAI_API_KEY) {
            return `Error: OpenAI API key not configured for ${modelName}`
          }
        ...
        case 'gemini-2.5-flash':
          if (!process.env.GOOGLE_GEMINI_API_KEY) {
            return `Error: Google Gemini API key not configured for ${modelName}`
          }
          const geminiResponse = await gemini.generateContentWithParams({
            prompt: prompt,
            temperature: 0.7,
            maxTokens: 1000
          })
      ...
      return `Error: Failed to generate response from ${modelName} - ${error instanceof Error ? error.message : 'Unknown error'}`
    }
  }
