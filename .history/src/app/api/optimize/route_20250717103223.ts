...
async function optimizePrompt(originalPrompt: string, requirements: string) {
  ...
  try {
    ...
    if (process.env.ANTHROPIC_API_KEY) {
      ...
      const optimizationPrompt = `You are a world-class prompt engineer. Your task is to rewrite and optimize the following user-provided prompt using expert prompt engineering practices.

      Original Prompt (detailed user input describing their intent, constraints, and expectations):  
      ...
      1. Be CLEAR & DIRECT — Avoid ambiguity, make instructions specific and actionable.
      2. Be OUTCOME-DRIVEN — Include explicit success criteria and expected output format.
      3. HANDLE EDGE CASES — Provide fallback instructions if inputs are missing or unclear.
      4. LEVERAGE MODEL STRENGTHS — Trust LLM capabilities, provide rich context where helpful.
      5. Be STRUCTURED & MAINTAINABLE — Use clean formatting, headings, bullet points, if applicable.
      6. COMMUNICATE INTENT EMPATHETICALLY — Make it understandable for both model and human readers.
      7. ENCOURAGE COLLABORATIVE OUTPUT — Where useful, instruct the model to check or self-assess its response.
      
      Important: At the end of your response, include a sample schema showing how the optimized prompt would be used, formatted like this:
      {
        "input": "Detailed user intent here...",
        "optimizedPrompt": "Final optimized prompt...",
        "expectedOutputFormat": "Example: Markdown paragraph, JSON object, bullet points, etc."
      }
      
      Respond with ONLY valid JSON in this format:
      {
        "optimizedPrompt": "your improved version of the prompt here",
        "explanation": "briefly explain what you changed and why",
        "changes": [
          {
            "type": "addition" | "removal" | "rewording" | "structure",
            "description": "what exactly changed",
            "reasoning": "why it helps"
          }
        ],
        "exampleSchema": {
          "input": "user's original task or question",
          "optimizedPrompt": "your optimized version",
          "expectedOutputFormat": "format or structure of model output"
        }
      }
      `;
